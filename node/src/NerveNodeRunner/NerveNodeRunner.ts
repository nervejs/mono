...
import {
	INerveNodeAppRunModule,
	INerveNodeRunnerCliOptions,
	INerveNodeRunnerProjectOptions,
	TProcessMessage,
} from './types';

const cluster = clstr as unknown as Cluster;

export class NerveNodeRunner extends NerveNodeObject {

	protected options: INerveNodeRunnerProjectOptions;

	constructor(options: INerveNodeRunnerProjectOptions) {
		super();
		this.options = {
			pathToProject: './src',
			appRunFileName: 'run',
	...
	init() {
		...
		const cliOptions = program.opts<INerveNodeRunnerCliOptions>();
		...
		if (cluster.isPrimary) {
			...
			const { run, createApp } = require(appRunPath) as INerveNodeAppRunModule;
			const app = createApp({
				http: {
	...
	static run(options: INerveNodeRunnerProjectOptions) {
		const instance = new NerveNodeRunner(options);
